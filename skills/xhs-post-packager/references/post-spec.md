...
```json
{
  ...
  "caption": {
    ...
    "hashtags": ["PPT模板", "PPT使用技巧", "演示设计", "AI工具", "开源项目", "GitHub项目", "效率工具", "设计工具", "职场汇报", "内容创作"]
  },
  "slides": [
    {
      "kicker": "Github高星好活儿",
      "headline": "封面钩子",
      "subhead": "可选副标题",
      "bullets": ["适用：让 coding agent 做职场汇报、课程 deck、产品方案页", "Last Update：2026-06-15 · 2.86k ★"],
      "footer": "可选页脚",
      "alt": "给图片的替代文本"
    }
  ]
...
- Set slide 1 `kicker` exactly to `Github高星好活儿`.
...
- The cover should include these metadata points in 2 short bullets:
  - `适用：<a concrete scenario or audience sentence>`; make it meaningful, not a compressed keyword list. It may wrap to a second line when needed.
  - `Last Update：<YYYY-MM-DD> · <checked star count> ★`; after the date, put the star count and a star icon directly. The renderer colors the `★` icon gold on the cover.
...
- Mix four kinds of tags:
  ...
  - Category tags: the field users would search, such as `PPT模板`, `开源项目`, `佛学智慧`, `文档解析`.
  - Audience/use-case tags: who or what scenario it helps, such as `职场汇报`, `论文阅读`, `关系内耗`.
  - Intent/problem tags: search phrases tied to the user's need, such as `PPT使用技巧`, `效率工具`, `情绪反刍`.
