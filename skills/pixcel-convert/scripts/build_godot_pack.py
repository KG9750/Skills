...
func verify_resources() -> void:
    ...
    if not ok:
        push_error("PIXCEL_CONVERT_GODOT_QA_FAIL")
        get_tree().quit(1)
        return
    print("PIXCEL_CONVERT_GODOT_QA_PASS tileset_sources=1 tiles=%s spriteframes=%s" % [ASSET_IDS.size(), ASSET_IDS.size()])
    get_tree().quit(0)
'''
...
def main() -> None:
    ...
    print(
        f"PIXCEL_BUILD_PASS pack={pack_name} assets={len(assets)} frames={len(atlas_records)} "
