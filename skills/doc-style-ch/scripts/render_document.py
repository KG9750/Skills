#!/usr/bin/env python3
"""Render structured JSON into a self-contained doc-style-CH HTML source."""
...
def safe_image_url(value: object, label: str) -> str:
    ...
    if (
        ...
        raise ValueError(f"{label} must stay within the delivery directory")
...
def render_block(block: object, section_index: int, block_index: int) -> str:
    ...
    if block_type == "code":
        ...
        language_value = require_string(
            block.get("language", "text"),
            label + ".language",
        )
        language_class = html.escape(language_value, quote=True)
        language_label = html.escape(f"<{language_value.strip().lower()}>")
        return (
            ...
            '</div>'
            f'<pre><code class="language-{language_class}">{code}</code></pre>'
            '</div>'
        )
...
def render_document(
    document: object,
    template: str,
    output_profile: str = "document",
) -> str:
    if output_profile not in {"document", "image"}:
        raise ValueError("output_profile must be document or image")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="document",
        help="document for responsive HTML/PDF, image for a fixed-canvas PNG source",
    )
    args = parser.parse_args()
    document = json.loads(args.input.read_text(encoding="utf-8"))
    template = args.template.read_text(encoding="utf-8")
    result = render_document(document, template, args.profile)
